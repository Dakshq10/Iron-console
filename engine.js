...
(function (root, factory) {
  ...
  function ex(name, type, primary, secondary, equip, inc, roles, sessions, alts, opt) {
    ...
    return {
      ...
      // Assist machines counter-weight you: MORE load = EASIER. Progression on
      // these lifts must move the number DOWN, and a deload moves it UP.
      inverse_load: !!opt.inverse,
  ...
  const LIBRARY = [
    ...
    ex("Barbell Bench Press", "compound", "chest", ["triceps", "front delt"], "barbell", 2.5, ["chest_press", "chest_flat"], ["backchest", "push"], ["Bench Press (Dumbbell)", "Machine Chest Press"]),
    ...
    ex("Incline Bench Press (Smith Machine)", "compound", "upper chest", ["front delt", "triceps"], "machine", null, ["chest_press", "chest_incline"], ["backchest", "push"], ["Incline Barbell Press", "Incline Dumbbell Bench Press"], { vtaper: true }),
    ...
    ex("Incline Barbell Press", "compound", "upper chest", ["front delt", "triceps"], "barbell", 2.5, ["chest_press", "chest_incline"], ["backchest", "push"], ["Incline Bench Press (Smith Machine)", "Incline Dumbbell Bench Press"], { vtaper: true }),
    ...
    ex("Pull Up (Assisted)", "compound", "lats", ["biceps"], "machine", null, ["back_vertical"], ["pull", "backchest"], ["Pull Up", "Lat Pulldown (Cable)"], { vtaper: true, grip: true, inverse: true }),
    ...
    ex("Bent Over Row (Barbell)", "compound", "back", ["biceps", "rear delt"], "barbell", 2.5, ["back_horizontal", "back_secondary"], ["pull", "backchest"], ["T-Bar Rows", "Dumbbell Rows"]),
    ...
    ex("Meadows Rows", "compound", "back", ["biceps", "rear delt"], "barbell", 2.5, ["back_horizontal", "back_secondary"], ["pull", "backchest"], ["T-Bar Rows", "Dumbbell Rows"]),
    ex("T-Bar Rows", "compound", "back", ["biceps", "rear delt"], "machine", null, ["back_horizontal", "back_secondary"], ["pull", "backchest"], ["Bent Over Row (Barbell)", "Meadows Rows"]),
    ...
    ex("Preacher Curl (Barbell)", "isolation", "biceps", [], "barbell", 2.5, ["biceps_heavy", "biceps_iso"], ["arms", "pull"], ["EZ-Bar Curls", "Incline Dumbbell Curls"]),
    ...
    ex("EZ-Bar Curls", "isolation", "biceps", [], "barbell", 2.5, ["biceps_heavy", "biceps_iso"], ["arms", "pull"], ["Preacher Curl (Barbell)", "Dumbbell Bicep Curls"]),
    ...
    ex("EZ-Bar Skull Crushers", "isolation", "triceps", [], "barbell", 2.5, ["triceps_iso", "triceps_heavy"], ["arms", "push"], ["Overhead Dumbbell Extensions", "Close-Grip Bench Press"]),
    ex("Close-Grip Bench Press", "compound", "triceps", ["chest"], "barbell", 2.5, ["triceps_iso", "triceps_heavy", "chest_press"], ["arms", "push", "backchest"], ["EZ-Bar Skull Crushers", "Weighted Dips"]),

    // ---- Legs: quads / glutes
    ex("Barbell Back Squat", "compound", "quads", ["glutes"], "barbell", 2.5, ["lower_compound"], ["legs"], ["Hack Squat", "Leg Press"]),
    ...
    ex("Romanian Deadlift (Barbell)", "compound", "hamstrings", ["glutes", "lower back"], "barbell", 2.5, ["posterior_chain"], ["legs"], ["Cable Pull-Throughs", "Lying Leg Curls"]),
    ...
    // ---- Glutes accessory
    ex("Hip Thrusts", "compound", "glutes", ["hamstrings"], "barbell", 2.5, ["posterior_chain"], ["legs"], ["Cable Pull-Throughs", "Romanian Deadlift (Barbell)"]),
  ...
  // §9d Resolve the load increment for a lift.
  // Priority: explicit per-exercise override → lifter-reported machine stack →
  // equipment default. Barbell and dumbbell both default to 2.5kg.
  const EQUIP_INCREMENT = { barbell: 2.5, dumbbell: 2.5, cable: 2.5, machine: 2.5, bodyweight: 2.5 };
  function incrementFor(exercise, reported) {
    const rep = reported && reported[exercise.name];
    // A machine entry may be a bare number (spacing) or {increment, base} for a stack.
    if (rep != null) {
      const v = (typeof rep === "object") ? rep.increment : rep;
      if (v != null && v > 0) return v;
    }
    if (exercise.increment != null) return exercise.increment;
    return EQUIP_INCREMENT[exercise.equipment] != null ? EQUIP_INCREMENT[exercise.equipment] : 2.5;
  }
  // Stacks only produce discrete weights. Snap a proposed load onto the real
  // plate positions so we never prescribe a weight the machine cannot make.
  // Rounds UP on a tie so a progression is never silently cancelled.
  function snapToStack(exercise, weight, reported) {
    if (weight == null) return weight;
    const rep = reported && reported[exercise.name];
    const step = incrementFor(exercise, reported);
    const base = (rep && typeof rep === "object" && rep.base != null) ? rep.base : 0;
    if (!step || step <= 0) return weight;
    const n = Math.round((weight - base) / step);
    const snapped = base + Math.max(0, n) * step;
  ...
  // Order sets WITHIN a session by their recorded index. Before set_index existed
  // this was insertion order, which broke as soon as rows synced out of order —
  // and set order is what every progression rule below depends on.
  function orderSets(sets) {
    return sets.slice().sort((a, b) => {
      const ai = a.set_index != null ? a.set_index : 9999;
      const bi = b.set_index != null ? b.set_index : 9999;
  ...
  function lastSessionSets(name, logs, beforeISO) {
    ...
    return { date: day, sets: orderSets(all.filter((l) => l.date === day)) };
  }
  // The one set every progression and deload decision is judged on.
  function setOne(sets) { const o = orderSets(sets || []); return o.length ? o[0] : null; }
  // §7 Volume load for one session of one lift.
  function sessionVolume(sets) {
    return (sets || []).reduce((a, s) => a + ((s.weight || 0) * (s.reps || 0)), 0);
  }
  // Per-session volume history for a lift, oldest first.
  function volumeTrend(name, logs, limit) {
    const byDay = {};
    logsFor(name, logs).forEach((l) => { (byDay[l.date] = byDay[l.date] || []).push(l); });
    const out = Object.keys(byDay).sort().map((d) => ({
      date: d,
      volume: sessionVolume(byDay[d]),
      sets: byDay[d].length,
      topWeight: Math.max.apply(null, byDay[d].map((s) => s.weight || 0)),
    }));
    return limit ? out.slice(-limit) : out;
  }
  // §7 Compare two sessions SET BY SET (index to index). Adding a set is never a
  // regression — extra sets are reported separately, not folded into the verdict.
  function compareSessions(currSets, prevSets) {
    const a = orderSets(currSets || []), b = orderSets(prevSets || []);
    const n = Math.min(a.length, b.length);
    const rows = [];
    for (let i = 0; i < n; i++) {
      const dw = (a[i].weight || 0) - (b[i].weight || 0);
      const dr = (a[i].reps || 0) - (b[i].reps || 0);
      rows.push({
        index: i, weight_delta: dw, reps_delta: dr,
        better: dw > 0 || (dw === 0 && dr > 0),
        worse: dw < 0 || (dw === 0 && dr < 0),
      });
    }
    return {
      rows,
      added_sets: Math.max(0, a.length - b.length),
      removed_sets: Math.max(0, b.length - a.length),
      volume_delta: sessionVolume(a) - sessionVolume(b),
      set_one: rows.length ? rows[0] : null,
    };
  }
  // Days since a lift was last trained. null = never.
  function daysSinceLift(name, logs, todayISO) {
    const l = logsFor(name, logs);
    if (!l.length) return null;
    return daysBetween(l[0].date, todayISO);
  }
  ...
  function prescribe(exercise, band, slotSets, logs, ctx) {
    ...
    const s1 = setOne(last.sets);                       // §8 the ONLY set that decides progression
    const s1Reps = s1 ? (s1.reps || 0) : 0;
    const s1Rpe = s1 && s1.rpe != null ? s1.rpe : null;
    const setsAtWW = last.sets.filter((s) => s.weight === ww);
    const bestReps = Math.max.apply(null, setsAtWW.map((s) => s.reps));

    // §8 READINESS: set one at/above target with RPE <=8. Later sets dropping off
    // reflects rest length and within-session fatigue, NOT readiness to progress,
    // so they are deliberately ignored here.
    const s1Cleared = s1Reps >= hi && (s1Rpe == null || s1Rpe <= 8);
    const s1Missed = s1 ? s1Reps < lo : false;
    const increasedLastTime = prev ? ww > workingWeight(prev.sets) : false;

    const last_time = {
      date: last.date,
      summary: orderSets(last.sets).map((s) => `${s.weight || "BW"}×${s.reps}`).join(", "),
      set_one: s1 ? { weight: s1.weight, reps: s1Reps, rpe: s1Rpe } : null,
      volume: sessionVolume(last.sets),
      compare: prev ? compareSessions(last.sets, prev.sets) : null,
    };
    ...
    if (ctx.reentryActive) {
    ...
    } else if (s1Cleared && !increasedLastTime) {
      const jump = ww > 0 ? inc / ww : 0;
      if (ww > 0 && jump > 0.10) {
        ...
        const dir = exercise.inverse_load ? -1 : 1;
        weight = isBodyweight ? ww : snapToStack(exercise, ww + dir * inc, ctx.machineIncrements);
        if (exercise.inverse_load) weight = Math.max(0, weight);
        target = lo;
        const realInc = isBodyweight ? inc : Math.round(Math.abs(weight - ww) * 100) / 100;
        note = isBodyweight ? `Add ${inc}kg or push reps`
             : exercise.inverse_load ? `Less assistance — −${realInc}kg counterweight`
             : `Load up — +${realInc}kg`;
        flag = "increase";
      }
    } else if (s1Cleared && increasedLastTime) {
      ...
      // Set one under the band — hold and rebuild. Deload only triggers on a
      // SECOND consecutive set-one miss (see forcedDeload below).
      weight = ww; target = clamp(s1Reps, lo, hi);
      note = "Set one missed the band — hold and rebuild"; flag = "freeze";
    }

    // §8b Stall detector: set one cleared target repeatedly but the load never moved.
    // Almost always a wrong increment or a stack we can't step — surface it.
    const stall = clearedWithoutIncrease(exercise.name, logs, [lo, hi]);
    if (stall >= 2 && flag !== "increase") {
      base.flags.push("config_check");
      base.config_hint = `Set one has hit the target ${stall} sessions running without the weight moving. Check the increment for this lift${exercise.equipment === "machine" || exercise.equipment === "cable" ? " — set the real stack spacing in Settings" : ""}.`;
    }

    // §14c extended-absence haircut (applied after the progression decision).
    if (ctx.absence && ctx.absence.mult < 1) {
      const m = exercise.inverse_load ? (2 - ctx.absence.mult) : ctx.absence.mult;
      weight = weight == null ? weight : roundTo(weight * m, isBodyweight ? 2.5 : 0.5);
    ...
    if (ctx.deloadWeek) {
      weight = weight == null ? weight : roundTo(weight * (exercise.inverse_load ? 1.1 : 0.9), isBodyweight ? 2.5 : 0.5);
    ...
    // Forced per-lift deload (§10). Set one only, twice consecutively, and never
    // after a training gap — a drop following time off is detraining, not failure.
    const fd = forcedDeload(exercise.name, logs, [lo, hi], ctx.todayISO);
    if (fd.deload) {
      // 10% off the working weight, snapped to what the machine can actually make.
      // On an assist machine a deload means MORE counterweight, not less.
      const cut = isBodyweight ? ww
        : snapToStack(exercise, ww * (exercise.inverse_load ? 1.1 : 0.9), ctx.machineIncrements);
      weight = weight == null ? weight : cut;
      target = clamp(s1Reps || lo, lo, hi);   // keep the reps you last managed — no reset
      if (!base.flags.includes("deload")) base.flags.push("forced_deload");
      note = exercise.inverse_load
        ? `Deload — more assistance (+10%). Set one missed twice running.`
        : `Deload −10% — set one missed twice running. Same reps, lighter bar.`;
      flag = "freeze";
    } else if (fd.suppressed) {
      base.flags.push("gap_suppressed");
      note = `${fd.gap} days since you trained this — holding, not deloading. ` + note;
    }
  ...
  // §10 forced deload trigger — SET ONE ONLY, two consecutive sessions.
  // Later-set failure never triggers a deload: it is a rest-and-fatigue artefact.
  // Suppressed entirely if the lift hasn't been trained in over 7 days.
  const DELOAD_GAP_DAYS = 7;
  function forcedDeload(name, logs, band, todayISO) {
    const [lo] = band;
    const byDay = {};
    logsFor(name, logs).forEach((l) => { (byDay[l.date] = byDay[l.date] || []).push(l); });
    const days = Object.keys(byDay).sort().reverse();
    if (days.length < 2) return { deload: false, suppressed: false, gap: null };

    // Detraining guard: a long layoff invalidates the comparison.
    if (todayISO) {
      const gap = daysBetween(days[0], todayISO);
      if (gap > DELOAD_GAP_DAYS) return { deload: false, suppressed: true, gap };
    }
    // And a gap BETWEEN the two sessions being compared is equally invalidating.
    const innerGap = daysBetween(days[1], days[0]);
    if (innerGap > DELOAD_GAP_DAYS) return { deload: false, suppressed: true, gap: innerGap };

    const missed = days.slice(0, 2).every((day) => {
      const s1 = setOne(byDay[day]);
      return s1 && (s1.reps || 0) < lo;
    });
    return { deload: missed, suppressed: false, gap: innerGap };
  }

  // §8b How many consecutive recent sessions had set one at/above target while the
  // working weight never moved. 2+ means the increment is probably misconfigured.
  function clearedWithoutIncrease(name, logs, band) {
    const [, hi] = band;
    const byDay = {};
    logsFor(name, logs).forEach((l) => { (byDay[l.date] = byDay[l.date] || []).push(l); });
    const days = Object.keys(byDay).sort().reverse().slice(0, 5);
    let n = 0, lastWW = null;
    for (const day of days) {
      const s1 = setOne(byDay[day]);
      const ww = workingWeight(byDay[day]);
      if (!s1 || (s1.reps || 0) < hi) break;
      if (lastWW != null && ww !== lastWW) break;   // the weight did move — not a stall
      lastWW = ww; n += 1;
    }
    return n;
  }
  ...
  return {
    ...
    incrementFor, snapToStack, workingWeight, lastSessionSets, logsFor, lastLogDate,
    orderSets, setOne, sessionVolume, volumeTrend, compareSessions, daysSinceLift,
    clearedWithoutIncrease, DELOAD_GAP_DAYS,
