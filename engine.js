  ...
  function prescribe(exercise, band, slotSets, logs, ctx) {
    ...
    if (ctx.reentryActive) {
      ...
      if (ww > 0 && jump > 0.10) {
        ...
        weight = isBodyweight ? ww : snapToStack(exercise, ww + inc, ctx.machineIncrements);
        target = lo;
        const realInc = isBodyweight ? inc : Math.round((weight - ww) * 100) / 100;
        note = isBodyweight ? `Add ${inc}kg or push reps` : `Load up — +${realInc}kg`;
        flag = "increase";
      }
    } else if (s1Cleared && increasedLastTime) {
    ...
    if (fd.deload) {
      ...
      const cut = isBodyweight ? ww : snapToStack(exercise, ww * 0.9, ctx.machineIncrements);
      ...
      note = `Deload −10% — set one missed twice running. Same reps, lighter bar.`;
      flag = "freeze";
