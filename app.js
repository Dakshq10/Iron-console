...
(function () {
  ...
  /* ============================================================ readout card */
  // §8 tiny sparkline of session volume for one lift
  function VolumeSpark({ trend }) {
    if (!trend || trend.length < 2) return null;
    const vals = trend.map((p) => p.volume);
    const min = Math.min.apply(null, vals), max = Math.max.apply(null, vals);
    const span = max - min || 1, W = 92, H = 22;
    const pts = vals.map((v, i) => `${(i / (vals.length - 1)) * W},${H - ((v - min) / span) * (H - 3) - 1.5}`).join(" ");
    const last = vals[vals.length - 1], prev = vals[vals.length - 2];
    const d = last - prev, pct = prev ? Math.round((d / prev) * 100) : 0;
    return html`
      <div class="g-volspark">
        <svg viewBox=${`0 0 ${W} ${H}`} width=${W} height=${H} preserveAspectRatio="none">
          <polyline points=${pts} fill="none" stroke="currentColor" stroke-width="1.6"/>
        </svg>
        <span class=${cls("delta", d > 0 && "up", d < 0 && "down")}>
          ${d === 0 ? "flat" : (d > 0 ? "+" : "") + pct + "%"}
        </span>
  ...
  function ReadoutCard({ line, onSwap, trend }) {
    ...
    return html`
      <div class="g-card" data-flag=${flag}>
        ...
        ${line.config_hint && html`<div class="g-configflag"><${Icon} d="alert" size=${14}/> ${line.config_hint}</div>`}
        ${line.flags && line.flags.includes("gap_suppressed") && html`<div class="g-help">Deload logic paused — a drop after a break is detraining, not failure.</div>`}
        ${line.last_time && html`
          <div class="g-lasttime">
            <span class="lbl">Last time · ${shortDate(line.last_time.date)}</span>
            <span class="val">${line.last_time.summary}</span>
            ${line.last_time.set_one && html`<span class="s1">set 1: ${line.last_time.set_one.reps} reps${line.last_time.set_one.rpe != null ? " @ RPE " + line.last_time.set_one.rpe : ""}</span>`}
          </div>`}
        ${trend && html`<${VolumeSpark} trend=${trend}/>`}
  ...
  function TodayScreen({ state, refresh, toast, onStart }) {
    ...
    return html`
      ...
      ${plan ? plan.items.map((line, i) => html`<${ReadoutCard} key=${i} line=${line} onSwap=${setSwap} trend=${E.volumeTrend(line.exercise, state.logs || [], 8)}/>`)
  ...
  function LiveLogging({ state, plan, onDone, onCancel, toast }) {
    const t = todayISO();
    // Per-set target RPE: the last set of each exercise is the one that decides
    // readiness, so it is taken to RPE 8 and its RPE entry is mandatory.
    const targetRpeFor = (i, n) => (i === n - 1 ? 8 : 7);
    const mkSets = (n, weight, reps) => Array.from({ length: n }, (_, i) => ({
      weight: weight == null ? "" : weight, reps: "", rpe: "", done: false, station: "",
      target_reps: reps, target_rpe: targetRpeFor(i, n), rpe_required: i === n - 1,
    }));
    const initial = useMemo(() => plan.items.map((it) => ({
      ...
      band: it.band, prescribed: it.exercise,
      target: `${it.sets}×${it.target_reps}`,
      target_reps: it.target_reps, prescribed_weight: it.weight,
      last_time: it.last_time || null, flags: it.flags || [], config_hint: it.config_hint || null,
      sets: mkSets(it.sets, it.weight, it.target_reps),
    })), [plan]);
    ...
    // Adding a set never rewrites earlier set indices, so it can never read as a
    // regression later: comparison is by index and extra sets are reported apart.
    const addSet = (ei) => setEntries((es) => es.map((e, i) => {
      if (i !== ei) return e;
      const prevLast = e.sets[e.sets.length - 1] || {};
      const sets = e.sets.map((x, j) => ({ ...x, rpe_required: false, target_rpe: j === e.sets.length ? 8 : x.target_rpe }));
      sets.push({ weight: prevLast.weight, reps: "", rpe: "", done: false, station: prevLast.station || "",
                  target_reps: e.target_reps, target_rpe: 8, rpe_required: true });
      return { ...e, sets };
    }));
    // §UI Swap a movement mid-session. The replacement is re-priced by the engine,
    // and the original is recorded so the session shows as "substituted".
    const [swapFor, setSwapFor] = useState(null);
    const swapExerciseAt = (ei, name) => {
      setSwapFor(null);
      const cur = entries[ei];
      const line = repriceLine(state, name, {
        band: cur.band || "isolation", sets: cur.sets.length,
        slot_label: cur.slot_label, is_lateral_delt: cur.is_lateral_delt, warmup: cur.warmup,
      });
      if (!line) { toast("Couldn't price that movement", "bad"); return; }
      setEntries((es) => es.map((e, i) => i !== ei ? e : {
        ...e, exercise: name, equipment: line.equipment,
        rest_low: line.rest_low, rest_high: line.rest_high,
        target: `${line.sets}×${line.target_reps}`, target_reps: line.target_reps,
        prescribed_weight: line.weight, last_time: line.last_time || null,
        flags: line.flags || [], config_hint: line.config_hint || null,
        substituted_from: e.substituted_from || e.prescribed,
        sets: mkSets(cur.sets.length, line.weight, line.target_reps),
      }));
      toast(`Swapped to ${name} — re-priced from your history`, "good");
    };
    const addExercise = (name) => {
      ...
      setEntries((es) => es.concat([{
        ...
        band: line.band, prescribed: null, target_reps: line.target_reps,
        prescribed_weight: line.weight, last_time: line.last_time || null,
        flags: line.flags || [], config_hint: line.config_hint || null, extra: true,
        sets: mkSets(line.sets, line.weight, line.target_reps),
      }]));
    };

    const finish = async () => {
      // §11 RPE is mandatory on the LAST logged set of each exercise — that is the
      // set the deload rule reads. Optional everywhere else.
      const missingRpe = [];
      entries.forEach((e) => {
        const logged = e.sets.filter((x) => parseInt(x.reps, 10) > 0);
        if (!logged.length) return;
        const lastSet = logged[logged.length - 1];
        if (lastSet.rpe === "" || lastSet.rpe == null) missingRpe.push(e.exercise);
      });
      if (missingRpe.length) {
        toast(`RPE needed on the last set of: ${missingRpe.join(", ")}`, "bad");
        return;
      }
      setSaving(true);
      const rows = [];
      entries.forEach((e) => {
        ...
        e.sets.forEach((s) => {
          ...
          rows.push({
            ...
            set_index: ix++,                       // explicit order — never inferred again
      ...
      // §13 record what actually happened, not just that something did.
      const subs = entries.filter((e) => e.substituted_from)
        .map((e) => ({ prescribed: e.substituted_from, performed: e.exercise }));
      const prescribedNames = plan.items.map((i) => i.exercise);
      const performedNames = entries.map((e) => e.exercise);
      const anyMissing = prescribedNames.some((n) => !performedNames.includes(n));
      const status = subs.length ? "substituted" : anyMissing ? "partial" : "as_prescribed";
      try {
        await Store.setSessionStatus({
          date: t, session_key: plan.session_key, status, substitutions: subs,
          note: anyMissing ? "Some prescribed movements were not logged" : "",
        });
      } catch (_) { /* status is a nicety — never block the save */ }
    ...
    return html`
      ...
      ${entries.map((e, ei) => {
        const lt = e.last_time;
        const ltSets = lt && lt.summary ? String(lt.summary).split(", ") : [];
        const needsStation = e.equipment === "cable" || e.equipment === "machine";
        return html`
        <div class="g-log-ex" key=${ei}>
          ...
          ${e.substituted_from && html`<div class="g-subbadge">substituted for ${e.substituted_from}</div>`}
          ${e.config_hint && html`<div class="g-configflag"><${Icon} d="alert" size=${14}/> ${e.config_hint}</div>`}
          ${lt && html`
            <div class="g-lasttime">
              <span class="lbl">Last time · ${shortDate(lt.date)}</span>
              <span class="val">${lt.summary}</span>
              ${lt.volume ? html`<span class="vol">${Math.round(lt.volume)} kg·reps</span>` : null}
            </div>`}
          ...
          ${e.sets.map((s, si) => html`
            <div class=${cls("g-set", s.done && "done", s.rpe_required && "lastset")} key=${si}>
              <div class="ix">${si + 1}</div>
              <div class="g-field"><label>kg</label>
                <input class="g-num" inputMode="decimal" value=${s.weight}
                  placeholder=${e.equipment === "bodyweight" ? "BW" : (s.weight === "" ? "—" : "")}
                  onChange=${(ev) => setField(ei, si, "weight", ev.target.value)}/></div>
              <div class="g-field"><label>reps</label>
                <input class="g-num" inputMode="numeric" value=${s.reps} placeholder=${String(s.target_reps)}
                  onChange=${(ev) => setField(ei, si, "reps", ev.target.value)}/></div>
              <div class="g-field"><label>RPE${s.rpe_required ? " *" : ""}</label>
                <input class=${cls("g-num", s.rpe_required && s.rpe === "" && "need")} inputMode="decimal" value=${s.rpe}
                  placeholder=${"≤" + s.target_rpe}
              ...
              <div class="g-settarget">
                target ${s.target_reps} @ RPE ${s.target_rpe}${si === 0 ? " · decides your next weight" : ""}
                ${ltSets[si] ? html`<span class="prev"> · last: ${ltSets[si]}</span>` : null}
              </div>
              ${needsStation && html`
                <div class="g-station">
                  <input placeholder="station / machine (e.g. left pulley, per-side)" value=${s.station || ""}
                    onChange=${(ev) => setField(ei, si, "station", ev.target.value)}/>
                </div>`}
            </div>`)}
          <div style=${{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            <button class="g-btn ghost sm" onClick=${() => addSet(ei)}><${Icon} d="plus" size=${14}/> Add set</button>
            <button class="g-btn ghost sm" onClick=${() => setSwapFor(ei)}>Swap movement</button>
          </div>
        </div>`; })}

      ${swapFor != null && html`
        <div class="g-overlay" onClick=${() => setSwapFor(null)}>
          <div class="g-modal" onClick=${(ev) => ev.stopPropagation()}>
            <h3>Swap ${entries[swapFor] && entries[swapFor].exercise}</h3>
            <div class="g-help">The replacement is priced from your own history for that movement. Anything you've already logged for this exercise stays.</div>
            <div class="g-chiprow" style=${{ maxHeight: "42vh", overflow: "auto" }}>
              ${(((E.LIB_BY_NAME[entries[swapFor] && entries[swapFor].exercise] || {}).alts) || [])
                .filter((n) => E.LIB_BY_NAME[n])
                .map((n) => html`<button key=${n} class="g-chip alt" onClick=${() => swapExerciseAt(swapFor, n)}>${n} <small>suggested</small></button>`)}
              ${E.LIBRARY.map((x) => html`<button key=${"all-" + x.name} class="g-chip" onClick=${() => swapExerciseAt(swapFor, x.name)}>${x.name}</button>`)}
            </div>
            <button class="g-btn block ghost" onClick=${() => setSwapFor(null)}>Cancel</button>
          </div>
        </div>`}
  ...
  // §13 what actually happened on a given day
  function outcomeFor(state, dateISO, sessionKey) {
    const rows = (state.sessionStatus || []).filter((r) => r.date === dateISO);
    const row = rows.find((r) => r.session_key === sessionKey) || rows[0];
    if (!row) return null;
    return row;
  }
  const OUTCOME_LABEL = {
    as_prescribed: { text: "as prescribed", cls: "good" },
    substituted:   { text: "substituted",   cls: "warn" },
    partial:       { text: "partial",       cls: "warn" },
    skipped:       { text: "skipped",       cls: "bad" },
  };
  function OutcomeBadge({ state, dateISO, sessionKey }) {
    const row = outcomeFor(state, dateISO, sessionKey);
    if (!row) return null;
    const m = OUTCOME_LABEL[row.status] || OUTCOME_LABEL.as_prescribed;
    const subs = (row.substitutions || []);
    return html`<span class=${"g-pill " + m.cls} title=${subs.map((x) => x.prescribed + " → " + x.performed).join(", ")}>
      ${m.text}${subs.length ? ` (${subs.length})` : ""}</span>`;
  }

  function DayModal({ day, onClose, onStart, t, state }) {
    ...
    return html`
      <div class="g-overlay" onClick=${onClose}>
        <div class="g-modal" onClick=${(e) => e.stopPropagation()}>
          ...
          ${(() => {
            const row = state ? outcomeFor(state, day.date, o && o.session) : null;
            if (!row) return null;
            const m = OUTCOME_LABEL[row.status] || OUTCOME_LABEL.as_prescribed;
            const subs = row.substitutions || [];
            return html`
              <div class="g-row"><span class="k">Performed</span><span class="v"><span class=${"g-pill " + m.cls}>${m.text}</span></span></div>
              ${subs.length ? html`<div class="g-help">${subs.map((x) => `${x.prescribed} → ${x.performed}`).join(" · ")}</div>` : null}
              ${row.note ? html`<div class="g-help">${row.note}</div>` : null}`;
          })()}
  ...
  // §12 Days since each muscle group was last trained, worst first.
  function muscleFreshness(state, t) {
    const byMuscle = {};
    (state.logs || [])
      .filter((l) => l.session_type !== "measurement")
      .forEach((l) => {
        const lib = E.LIB_BY_NAME[l.exercise];
        if (!lib || !lib.primary) return;
        const m = lib.primary;
        if (!byMuscle[m] || l.date > byMuscle[m]) byMuscle[m] = l.date;
      });
    // every muscle the library trains, so gaps are visible even at zero sessions
    const all = {};
    E.LIBRARY.forEach((x) => { if (x.primary) all[x.primary] = true; });
    return Object.keys(all).map((m) => ({
      muscle: m,
      last: byMuscle[m] || null,
      days: byMuscle[m] ? E.daysBetween(byMuscle[m], t) : null,
    })).sort((a, b) => {
      const av = a.days == null ? 9999 : a.days, bv = b.days == null ? 9999 : b.days;
      return bv - av;
    });
  }

  function MuscleFreshness({ state, t }) {
    const rows = useMemo(() => muscleFreshness(state, t), [state.logs, t]);
    const stale = rows.filter((r) => r.days == null || r.days > 14);
    return html`
      <div class="g-card" style=${{ marginBottom: 12 }}>
        <div class="g-cardhead">
          <span>Muscle freshness</span>
          ${stale.length ? html`<span class="g-pill bad">${stale.length} over 14 days</span>` : html`<span class="g-pill good">all current</span>`}
        </div>
        <div class="g-freshgrid">
          ${rows.map((r) => html`
            <div class=${cls("g-fresh", (r.days == null || r.days > 14) && "stale", r.days != null && r.days > 7 && r.days <= 14 && "warn")} key=${r.muscle}>
              <span class="m">${r.muscle}</span>
              <span class="d">${r.days == null ? "never" : r.days === 0 ? "today" : r.days + "d"}</span>
            </div>`)}
        </div>
        <div class="g-help">Anything over 14 days is highlighted. "Never" means no logged session for that muscle yet.</div>
      </div>`;
  }
  ...
  function PlanScreen({ state, toast, onStart }) {
    ...
    return html`
      ...
      <${MuscleFreshness} state=${state} t=${t}/>
      ...
      ${dayModal && html`<${DayModal} state=${state} day=${dayModal} t=${t} onClose=${() => setDayModal(null)}
